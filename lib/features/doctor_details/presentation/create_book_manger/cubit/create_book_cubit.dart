...
class CreateBookCubit extends Cubit<CreateBookState> {
  CreateBookCubit({required this.createBookRepo}) : super(CreateBookintialState());
final  CreateBookRepo createBookRepo;
 Future<void> create({required int doctorId,required String appointmentDate,required String appointmentTime})async
  {
emit(CreateBookLoadingState());
var result=await createBookRepo.createBook(doctorId: doctorId, appointmentDate: appointmentDate, appointmentTime: appointmentTime);
result.fold((failure){
